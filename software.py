...
class BankUi:
    ...
    def enroll_and_move_to_next_screen(self):
        ...
        if not name and not password:
            return jsonify({"message": "You need a name to enroll an account and you need to input a password!"}), 400
        elif not password:
            return jsonify({"message": "You need to input a password!"}), 400
        elif not name:
            return jsonify({"message": "You need a name to enroll an account!"}), 400
        elif len(password) < 8:
            return jsonify({"message": "Your password needs to be at least 8 digits!"}), 400
        else:
            self.write_to_csv()
    ...
    def user_account_transfer_transc(self):
        ...
        if recipient_account == data[data['unique_id'] == self.real_user]['account_number'].values[0]:
            return jsonify({"message": "You cannot transfer money to yourself"}), 400
    ...
    def user_balance(self):
        ...
        if pd.isnull(user_balance):
            return jsonify({"message": "Your account balance is not initialized. Please deposit money first."}), 400
        else:
            return jsonify({"message": f"Your current balance is: Rs.{user_balance}"}), 200
    ...
    def user_deposit_trans(self):
        data = pd.read_csv('bank_details.csv')
        deposit_input = self.money_box.get()
        
        if not deposit_input:  # Check if the input is empty
            return jsonify({"message": "Please enter an amount to deposit."}), 400
        
        try:
            deposit_amount = int(deposit_input)  # Convert to integer
        except ValueError:
    ...
    def captureuser(self):
        ...
        self.get_embeddings()
        self.train_model()
        return jsonify({"message": "Face Id Successfully Registered!"}), 200
    ...
    def video_check(self):
        ...
        try:
            ...
            if self.countter != 0:
                return jsonify({"message": "Face Id match failed! You have {} trials left".format(self.countter)}), 403
                self.countter = self.countter - 1
                self.video_check()
            else:
                return jsonify({"message": "Face Id match failed! You cannot withdraw at this time, try again later"}), 403
                self.begin_page()
                self.countter = 2
            ...
            if Counter(real_user_list).most_common(1)[0][0] == 'unknown':
                ...
                return jsonify({"message": "Face Id match!"}), 200
