...
class BankUi:
    ...
    def enroll_and_move_to_next_screen(self):
        ...
        if not name and not password:
            ...
            messagebox._show("Error", "You need to name to enroll an account!")
            self.enroll_user()
        elif len(password) < 8:
    ...
    def train_model(self):
        ...
        # Ensure that embeddings and labels are valid
        if len(data["embeddings"]) == 0 or len(labels) == 0:
            raise ValueError("No embeddings or labels found for training.")
